...
- 镜 8 源为 800x450 Playwright 录屏，放大 2.4x 有轻微软化；字幕已注明 F1 链路与 JUnit 结果，末帧未嵌入 junit XML 画面（分镜的"末帧切 junit"简化掉，口播可补）。备答口径见 pitch-and-demo.md §2.2 成片备答。
- 镜 8（MiniShop 搜索）与镜 1-6（demo_form 表单）是两条不同的已验收链路，字幕已注明"另一条已验收链路 F1"（红队 S5 修复）；被追问的备答在 pitch §2.2。
- 无音轨（用户决定：视频+字幕，不要旁白）。

## 6. 红队复审联动（2026-09-24）

红队报告（red-team-report.md）S5 建议落地：镜 8 字幕由"真实执行（已固化录屏）…"改为"另一条已验收链路 F1：录制 MiniShop 搜索 → 蒸馏 → 真实执行 62s 通过（JUnit tests=1 errors=0）"，已重新合成并抽查（t=87s 帧字幕完整无裁切）。
